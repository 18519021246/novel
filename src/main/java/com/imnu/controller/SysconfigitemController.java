...
public class SysconfigitemController extends SimpleController {
	...
	public void load() {
		...
		if (id != null) {
			Sysconfigitem sysconfigitem = sysconfigitemSrv.load("where id="
					+ id);
			if (sysconfigitem != null) {
	...
	public void get() {
		...
		List<Sysconfigitem> listsysconfigitem = sysconfigitemSrv
				.getPageEntitys(filter, pageindex, pagesize);
		int recordscount = sysconfigitemSrv.getRecordCount(filter == null ? ""
				: filter);
